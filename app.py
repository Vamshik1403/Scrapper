...
def outreach_send_bulk():
    ...
    thread = threading.Thread(target=_bulk_send_emails, args=(send_list,))
    thread.start()

    return jsonify({"status": "started", "msg": f"Sending {len(send_list)} emails...", "total": len(send_list)})


def _bulk_send_emails(send_list):
    """Background thread: send emails one by one with delay."""
    sent = 0
    failed = 0
    errors = []
    for item in send_list:
        ...
        if ok:
            ...
            failed += 1
            errors.append(f"{item['business_name']}: {err}")
        job_status["outreach_msg"] = f"Sent {sent}/{len(send_list)} emails... ({failed} failed)"
        time.sleep(2)  # Rate limit: 1 email every 2 seconds
    job_status["outreach"] = "done"
    msg = f"Done! Sent {sent}/{len(send_list)} emails."
    if failed:
        msg += f" {failed} failed."
